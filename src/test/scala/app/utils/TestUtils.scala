...
class TestUtils extends FlatSpec with SparkSessionTestWrapper {
  ...
  "A Utils" should "write and read xml parameter file" in {
    ...
    val sinkOption2 = new Option("checkpointLocation", "src\\main\\resources\\checkpoint")
    ...
    val transform2Column1  = new Column(_from = "Entry DtTm", _to = "EntryDtTs")
    val transform2Column2  = new Column(_from = "Received DtTm", _to = "ReceivedDtTs")
    val transform2Column3  = new Column("Entry DtTm", "EntryDtTs")
    val transform2Column4  = new Column("Received DtTm", "ReceivedDtTs")
    val transform2Column5  = new Column("Dispatch DtTm", "DispatchDtTs")
    val transform2Column6  = new Column("Response DtTm", "ResponseDtTs")
    val transform2Column7  = new Column("On Scene DtTm", "OnSceneDtTs")
    val transform2Column8  = new Column("Transport DtTm", "TransportDtTs")
    val transform2Column9  = new Column("Hospital DtTm", "HospitalDtTs")
    val transform2Column10 = new Column("Available DtTm", "AvailableDtTs")
    val transform2 =
      new Transform(
    ...
    val url = new SqlFileLocation("src\\test\\resources\\transformation.sql", "data")

    val result = new Parameter(source, sink, List(transform1, transform2), url)

    XML.save("src\\test\\resources\\parameter.xml", result.toXML, "UTF-8", xmlDecl = true)
